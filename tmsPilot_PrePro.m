function [] = tmsPilot_PrePro(id,condition,N,TR,TA,pathName)
...
% condition = 'string'; TMS condition. Example: 'HI1'.
% N = integer; Number of acquisitions. Example: 119

% Dependencies

% The script assumes EPI files in .nii format for each TMS sessions included 
% in a folder titled by the session name. In addition, corresponding T1
% files in .nii format are required in a folder named 'T1'.Both folders are required 
% in a folder named as ['pilot',id]. The folder should be within the
% following path: /gpfs/M2Home/projects/Monash076/Nigel/TMS-fMRI/

% Created by Nigel Rogasch, 2-11-2016

spm_jobman('initcfg')

%-----------------------------------------------------------------------
% Job saved on 02-Nov-2016 08:59:35 by cfg_util (rev $Rev: 6134 $)
% spm SPM - SPM12 (6225)
%-----------------------------------------------------------------------
%%
...
for x = 1:N
    matlabbatch{1}.spm.temporal.st.scans{1,1}{x,1} = [pathName,id,'/',id,'_',condition,'/',id,'_',condition,'.nii,',num2str(x)];
end
...
% 3. Coregistration of EPI to T1
t1Ref = [pathName,id,'/',id,'_T1/',id,'_T1.nii,1'];
