...
for ii=1:numscans
    ...
    if strcmp(MRS_struct.p.target,'GABA')
        ...
        %chop freqbounds CHAO Josh
        
%         cut_up_freq=3.5;  %in ppm
%         cut_down_freq=3.2;     %in ppm
%         
...
%         
        ...
        %The above code fits the gaussian model, and spits out
        %GaussModelParam (the red line you see fit to the data after
        %running GannetFit
        
       
        %Pasted from part 1A below:
        
 %+++++++++++added++++++++++  
        [a b c]=fileparts(MRS_struct.gabafile{1});
        save (['temp_' b '.mat'])
%         delete('all.mat')
        %save ('all2.mat')
        %Fit_Gui
%         Fit_Gui_with_convergence
%         figure(999)
...
%         Fit_Gui(GaussModelParam,freq,freqbounds,plotbounds,GABAData);
        
        %ADD A SLIDER TO BE ABLE TO TWEAK THE VALUES OF
        %GaussModelParam(ii,:) IF NECESSARY.
        %Best to have some metric (see the next 10 lines of code - e.g.
        %GABAFitError, that helps to identify if the trial needs to be
        %"fixed"
        %Code resumes here
